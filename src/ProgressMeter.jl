...
struct ProgressElements
    elements::Tuple{Vararg{AbstractProgressElement}}
end
...
mutable struct Progress <: AbstractProgress
    ...
    function Progress(
            ...
            elements::ProgressElements=defaultprogresselements,
            kwargs...)
        CLEAR_IJULIA[] = clear_ijulia()
        core = ProgressCore(;kwargs...)
        new(n, start, barlen, elements, core)
    end
end
...
function _updateProgress!(p::Progress; ignore_predictor = false, force::Bool = false, kwargs...)
    if p.counter == 2
        p.tsecond = time()
    end
...
function printprogresselements(p::Progress)
